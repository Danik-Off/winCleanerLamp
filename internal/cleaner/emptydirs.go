...
// IsPathSafeToDelete (safety.go) + DeleteDir (delete.go). Здесь раньше была
// третья, отдельная от cleaner.go реализация проверки безопасности и своя
// PowerShell-логика перемещения в Корзину — обе убраны как дублирующие.
