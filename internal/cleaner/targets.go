...
// всех ОС — база остатков у каждой ОС своя (orphaned_apps.windows.json,
// orphaned_apps.linux.json, orphaned_apps.darwin.json), но чужой файл,
// подсунутый через --orphan-config, должен читаться, а не ломаться.
...
// windows-путь превращался бы в путь от корня диска.
//
