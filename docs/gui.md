...
Версия в установщике берётся из **`gui/package.json`** → поле `version`. Для согласованности с релизом рекомендуется держать её **в линии** с корневым [`package.json`](../package.json) в репозитории.
